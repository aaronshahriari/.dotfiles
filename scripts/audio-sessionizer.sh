...
selected_sink=$(echo "$sinks" | cut -d' ' -f2- | awk '{for(i=1;i<=NF;i++) $i=toupper(substr($i,1,1)) substr($i,2)}1' | dmenu -m 0 -i -g 1 -p "Select Sink:")
...
selected_source=$(echo "$sources" | cut -d' ' -f2- | awk '{for(i=1;i<=NF;i++) $i=toupper(substr($i,1,1)) substr($i,2)}1' | dmenu -m 0 -i -g 1 -p "Select Source:")
