...
get_sources() {
    wpctl status | awk '
    /Audio/,/Video/ {
        if ($0 ~ /Sources:/) in_sources=1
        else if ($0 ~ /Filters:/) in_sources=0
        else if (in_sources && $0 ~ /^[ │\*]*[0-9]+\./) {
            gsub(/^[ │\*]+/, "", $0)
            print
        }
    }'
...
selected_sink=$(echo "$sinks" | cut -d' ' -f2- | awk '{for(i=1;i<=NF;i++) $i=toupper(substr($i,1,1)) substr($i,2)}1' | ~/.config/rofi/launchers/scripts/launcher.sh "Sink:")
...
selected_source=$(echo "$sources" | cut -d' ' -f2- | awk '{for(i=1;i<=NF;i++) $i=toupper(substr($i,1,1)) substr($i,2)}1' | ~/.config/rofi/launchers/scripts/launcher.sh "Source:")
